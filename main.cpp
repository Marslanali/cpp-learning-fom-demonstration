 ...
 *
 * C++ Implementation of gaussian mixture model and gaussain mixture regression
 * for learning 3-dof task. The data set contains mutliple demonstrations of the
 * task which are trained using gaussian mixture model. Gaussian Mixture
 * Regression had been used for generating single generalized trajectory with
...
#include "em-Initilization-mlpack.h"
#include "em-Initilization.h"
...
{
    // Create a smart pointer for Datapoints class
  ...
  */
    ...
    // Create a smart pointer of GMM class
    std::shared_ptr<GMM> gmm = std::make_shared<GMM>();

    gmm->EM(position_data->get_data_points(),
            em_init_kmean_mlpack->get_priors(),
            em_init_kmean_mlpack->get_mu(),
            em_init_kmean_mlpack->get_sigma());
    ...
    // we can get learned generalized trajectory using Gaussian Mixture Regression
    // which time variable as an input and mean, covariance matrix as ouptut. The
    // time variable can be generated using linspace

    arma::Mat<double> clock_signal =
        arma::linspace<arma::Col<double>>(min(position_data->get_data_points()(0, arma::span::all)),
                                          max(position_data->get_data_points()(0, arma::span::all)),
                                          position_data->get_num_points());
    clock_signal = clock_signal.t();

    std::cout << "Clock signal :\n" << clock_signal << std::endl;
    ...
    std::cout << "Expected Mean:
                 "<<gmr->return_expected_mu()<<std::endl; std::cout<<" Expected Mean Size
        : "<<size(gmr->return_expected_mu())<<std::endl;*/
