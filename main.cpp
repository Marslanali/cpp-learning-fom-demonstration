...
int main(int argc, char **argv) {
  ...
  */
  ...
  for (int i = 0; i < gmm->return_mu().size(); ++i) std::cout << "Final Mu are :\n" << gmm->return_mu()[i] << std::endl;
  for (int i = 0; i < gmm->return_sigma().size(); ++i) std::cout << "Final Sigma are:\n" << gmm->return_sigma()[i] << std::endl;
  ...
  */
  ...
  for (int i = 0; i < nb_states; i++) file_kmean << em_init_kmean_mlpack->get_priors()[i] << " ";
  file_kmean << std::endl;
  ...
  // file << 2 << " ";
  // file << nb_states << std::endl;
  for (int i = 0; i < nb_states; i++) file_gmm << gmm->return_priors()[i] << " ";
  file_gmm << std::endl;
  for (int s = 0; s < nb_states; s++) {
    for (int i = 0; i < dim; i++) {
  ...
  */
  ...
  for (int i = 0; i < position_data->get_num_points(); ++i) plot_clock.push_back(clock_signal(0, i));
