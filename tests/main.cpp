...
int main(int argc, char **argv) {
  ...
  // std::cout<<"Time:
  // "<<position_data.get_data_points()(0,span::all)<<std::endl;
  ...
  */
  ...
  gmm->EM(position_data->get_data_points(), em_init_kmean_mlpack->get_priors(), em_init_kmean_mlpack->get_mu(),
  ...
  arma::Mat<double> clock_signal = arma::linspace<arma::Col<double>>(
      min(position_data->get_data_points()(0, arma::span::all)),
      max(position_data->get_data_points()(0, arma::span::all)), position_data->get_num_points());
  ...
  gmr.Compute_GMR(gmm.return_priors(), gmm.return_mu(), gmm.return_sigma(),
  clock_signal,in, out); std::cout<<"Expected Mean:
  ...
  */
  ...
  /*
      ...
      std::ofstream
     file_gmr("/home/arslan/CLionProjects/cpp_learning_from_demonstration/data/gmr/gmr.txt");
      //file << 2 << " ";
      //file << nb_states << std::endl;
      for(int s=0;s<nb_states;s++) {
          for(int i=0;i<dim;i++) {
