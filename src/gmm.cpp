...
void GMM::EM(const arma::Mat<double>& _data, const arma::Col<double>& _priors, const std::vector<arma::Col<double>>& _mu, const std::vector<arma::Mat<double>>& _sigma) {
  ...
  for (int iter = 0; iter < nb_max_steps; ++iter) {
    ...
    if (iter >= nb_min_steps) {
      if (log_likelihoods[iter] - log_likelihoods[iter - 1] < max_diff_log_likelihood) break;
    }
  }
...
arma::Col<double> GMM::return_priors() { return priors; }

std::vector<arma::Col<double>> GMM::return_mu() { return mu; }

std::vector<arma::Mat<double>> GMM::return_sigma() { return sigma; }