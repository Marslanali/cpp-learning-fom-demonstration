...
arma::Mat<double> Datapoints::get_data_points() { return data; }

uint Datapoints::get_num_vars() { return nb_vars; }

uint Datapoints::get_num_points() { return nb_points; }
...
void Datapoints::load_from_file(std::string path, bool is_transpose) {
  ...
  if (is_transpose) data = data.t();
  set_data(data);
}

void Datapoints::save_file(std::string path) { data.save(path, arma::raw_ascii); }
